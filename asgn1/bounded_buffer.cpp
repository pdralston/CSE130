...
ssize_t Bounded_Buffer::from_file(int file_d, ssize_t size) {
    ...
    size_t bytes_rem = size - bytes_read;
    flush();
    while (bytes_rem > 0) {
...
ssize_t Bounded_Buffer::to_file(int file_d, ssize_t size) {
    ssize_t bytes_written = 0, current_size = end - start, curr_bytes_rd; 
    while (bytes_written < 0) {
        ...
        curr_bytes_rd = write(file_d, start, current_size < size ? current_size : size);
        if (curr_bytes_rd == -1) {
            return -1;
        }
        bytes_written += curr_bytes_rd;
        start += curr_bytes_rd;
...
int8_t Bounded_Buffer::pushBytes(size_t size, uint8_t* in_bytes) {
    for (size_t i = 0; i < size; ++i) {
        if(pushByte(in_bytes[i]) == -1)  return -1;
    }
    return 0;
}